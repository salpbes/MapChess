...
## D-069 — Famous ground, presented: a card, sections, an arrival, and a board in the picker

**Date:** 2026-09-23 · **Phase:** beyond 13 · **Builds on:** D-068

Four changes to how the places in D-068 are offered, and one to the map picker for everyone.

**The menu's door is a card.** It was a quiet text button, _"Or start on famous ground"_, and a quiet button is exactly what a newcomer does not press — nothing on it said that Thermopylae and Monte Cassino were behind it. It is now a gold-edged card, _"Play on famous ground"_, with four real names under it, reshuffled each time the menu opens so a returning player notices somewhere new. The names do the persuading rather than an exclamation mark. It sits below the map row and below the weight of "New game" on purpose: anywhere on Earth is still the game.

**The list is in sections.** Ancient & medieval · 18th & 19th centuries · First World War · Second World War · Ready offline. Fifteen places in one run is a list you read; five short sections is one you scan, and a player who wants the Western Front looks for "First World War" before they know the name Douaumont. Headings stick while the list scrolls, and each section is a labelled `group` so a screen reader announces where it is. "Ready offline" is not an era, but it is how someone on a train needs those three sorted.

**Arriving at a chosen place is marked.** When its board is actually ready — not when the button is pressed, and not before a failed load is retried — a title rises over it: the era in spaced capitals, the place large, a hairline, the one line in italic. It holds for about four seconds and fades on its own. It never takes a pointer, and that was a real bug caught by its test: `#ui > *` hands pointer events back to every overlay and an id outranks a class, so the first version sat over the board swallowing taps for five seconds after every arrival. On a phone the list lives in the drawer, so choosing a place now also shuts the drawer — otherwise the title arrived over a drawer covering the ground you had just chosen.

**A battlefield's gazetteer is headed with the battle.** This reverses the call left open in D-068. Choosing Monte Cassino headed the entry _The field of San Silvestro_, which was accurate and flat; the player chose a battle. The heading is now _Monte Cassino_, the first line is its history, and the map's own name is kept one line down as _Field: San Silvestro_ rather than thrown away. Only for battlefields: the three quiet places keep their map names, so the default Holy Island board reads exactly as it did. Which place a board is comes from its area (`curatedPlaceAt`), exact to the metre, so a resumed game is named correctly with nothing extra saved, and a square dragged fifty metres off the mountain stops claiming the name.

**The picker draws a chessboard.** It used to show a yellow square and a sentence saying the bright edge was White's — a rule the player had to hold in their head and rotate. It now draws the 64 squares and both armies in their starting places, kings marked, inside the square, every corner through the same `AreaProjection` the game builds its board from, so each drawn square lies over exactly the ground that square will be and the whole board turns with the rotation slider. Pieces are discs rather than chess glyphs because a symbol layer would need those characters from the map style's font server, which serves Latin text and nothing so specific. Squares at 32% opacity, so roads and names still read through: the ground is what is being chosen.

**What the full suite caught, and why both fixes are the right shape.** The card made the menu nine pixels taller, and on a phone lying on its side (390 px) that put _Back to the board_ — the one way out — below the fold, the exact regression `layout.spec.ts` exists for. Short screens now get the card on one line, teaser dropped, door kept. And the title first carried its own `aria-live` region, which broke the Announcer's stated rule of _one region, not two_: two regions race, and a reader mid-sentence announces only whichever wrote last. The card is now `aria-hidden`, and the arrival is spoken through `Announcer.say()`, made public for the few things that are not game events but still need saying. The test asserts there is exactly one live region.

**Not built: pieces per era.** Worth designing for, not worth building before there are models. The shape it would take: `CuratedPlace` gains an optional `pieceSet`, sets live as `src/chesspieces/<set>/` with their own `compressed/`, and `loadPieceModels` takes the set name; `curatedPlaceAt` already tells a board which place it is, so nothing new would need to be saved. The one real cost is weight — every set is another ~11 MB — so a set should load when its place is chosen, never with the page.

## D-070 — Piece sets, by folder, named by army

**Date:** 2026-09-25 · **Phase:** beyond 13 · **Builds on:** D-069's sketch of per-era pieces

**Layout.** The author moved the original twelve masters into `src/chesspieces/medieval/`, their compressed copies into `compressed/medieval/`, and started `ww1/` with an ANZAC and an Ottoman pawn. A set is a folder rather than a filename prefix so it can be added, replaced or deleted as a unit. `npm run pieces` walks every set folder, and now skips any model whose compressed copy is newer than its master: at about nine seconds a model and several sets of twelve on the way, a script that re-encodes everything is a script people stop running, and that is how a changed master ends up serving its old compression. The loader globs `compressed/*/*.glb`, which costs nothing at page load — an eager `?url` glob is a table of strings — and fetches only the set asked for.

**The move broke the game locally before any code changed.** The loader read `compressed/*.glb`, which after the move matched nothing, so the working copy had silently fallen back to drawn pieces everywhere. It had not been pushed. The live site was never affected.

**Named by army.** `anzac_pawn.glb`, not `pawn_white.glb`. What the modeller knows while making a figure is whose soldier it is; which side of a chessboard that army sits on is a decision about the game, and it is made once, in `pieceSets.ts`. The filename's two words are accepted either way round, so the original set's `rook_white.glb` still parses. An army a set does not field — a French pawn in the Gallipoli folder — is skipped rather than guessed at.

**Scope, as decided by the author.** One `ww1` set for all three First World War boards for now, with Gallipoli's armies: ANZAC plays White, Ottoman plays Black. The side that landed and attacked moves first.

**Anzac Cove, re-sited to match.** The board is turned to 100° and moved 1.4 km inland so that White's back rank is the beach they landed on. The original coordinate was offshore: sampling the terrain along White's edge showed its back rank still in the sea a full kilometre inland. At the new site five of White's eight back-rank squares stand 6–16 m above the water, Black's line runs 99–212 m up the ridges, and the survey measures 198 m of relief and 56 named places, against 74 m and 22 before. The board confirms itself: the ANZAC Commemorative Site, on the beach, falls on e1, and The Nek, Baby 700 and Battleship Hill run up to Black's side. Lone Pine is on it, under its Turkish name, Kanlısırt.

**Hidden until whole, previewable at once.** A set reaches players only when all twelve pieces exist (`isCompleteSet`, read from the bundle's file list without fetching anything). An ANZAC pawn beside a medieval knight is a work in progress, not a board. `?pieces=<set>` previews any known set on every board, filling its gaps from `medieval` so the preview is always a full game; an unknown name is ignored rather than failed on, so a typo leaves the ordinary game.

**Where the choice lives.** `choosePieceSet` in `src/app/pieceSetChoice.ts`, pure and handed its facts, so the two promises — never a half-made set for a player, always the era's set on its own battlefield once whole — are unit-tested without a model on disk. The loading moved from `main.ts` into bootstrap, which is what knows which board it is building. Each set is fetched once and kept, and a board switching sets swaps the whole template map (`ModelPieceFactory.replace`), because walking from Anzac Cove back to Holy Island needs the Gallipoli figures gone. Every model carries its set in `userData`, which is how `pieceSets.spec.ts` asks a live board which pieces are standing on it.

**Not yet.** Verdun and Kobarid will want their own armies; the set would then move from per-era to per-battle, and `choosePieceSet` is the one function that changes.
