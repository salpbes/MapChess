...
**What the full suite caught, and why both fixes are the right shape.** The card made the menu nine pixels taller, and on a phone lying on its side (390 px) that put *Back to the board* — the one way out — below the fold, the exact regression `layout.spec.ts` exists for. Short screens now get the card on one line, teaser dropped, door kept. And the title first carried its own `aria-live` region, which broke the Announcer's stated rule of *one region, not two*: two regions race, and a reader mid-sentence announces only whichever wrote last. The card is now `aria-hidden`, and the arrival is spoken through `Announcer.say()`, made public for the few things that are not game events but still need saying. The test asserts there is exactly one live region.
